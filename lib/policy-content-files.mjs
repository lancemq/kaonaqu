...
const POLICY_CONTENT_DIR = path.join(process.cwd(), 'content', 'news', 'policy');
...
export function getPolicyContentRelativePath(id) {
  const item = typeof id === 'object' ? id : { id };
  const rawId = String(item?.id || '').trim();
  const rawTitle = String(item?.title || '').trim();
  const fileStem = /^policy-\d+$/i.test(rawId) && rawTitle ? rawTitle : rawId;
  return `content/news/policy/${fileStem}.md`;
}
...
export function readPolicyPlainText(itemOrId) {
  const markdown = readPolicyMarkdownFile(itemOrId);
