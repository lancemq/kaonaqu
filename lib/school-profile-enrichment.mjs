...
function getOwnershipLabel(school) {
  const text = cleanText(school.schoolPropertyLabel || school.schoolKeyLevel);
...
export function buildDecisionTags(school) {
  ...
  const admissionInfoText = school.admissionInfo?.notes || '';
  if (String(admissionInfoText).includes('对口') || String(admissionInfoText).includes('学区')) tags.push('对口入学');
...
export function buildProfileSignals(school) {
  ...
  return {
    ...
    comparePriority: score >= 80 || PRIORITY_TIERS.has(cleanText(school.schoolKeyLevel)) || PRIORITY_TIERS.has(cleanText((school.schoolKeyLevel || '').replace(/[（(].*?[)）]/g, ''))) ? 'high' : score >= 60 ? 'medium' : 'foundation',
...
function scoreRelatedCandidate(current, candidate) {
  ...
  if (current.schoolKeyLevel && current.schoolKeyLevel === candidate.schoolKeyLevel) score += 10;
