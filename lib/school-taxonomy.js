...
import { getSchoolOverview, getSchoolAchievementsText } from './school-content.js';
...
// 根据 eliteCohort/schoolKeyLevel/schoolPropertyLabel 推断 category
// （原 tier 字段已拆分为 eliteCohort 与 schoolKeyLevel，tier 列已从数据库删除）
export function inferSchoolCategory(school) {
  const eliteCohort = String(school?.eliteCohort || '').trim();
  const schoolKeyLevel = String(school?.schoolKeyLevel || '').trim();
  const schoolPropertyLabel = String(school?.schoolPropertyLabel || '').trim();
  const schoolStage = String(school?.schoolStage || '').trim();
  const isInternational = school?.isInternational || schoolPropertyLabel.includes('外籍');

  // 外籍人员子女学校优先级最高
  if (schoolPropertyLabel.includes('外籍')) {
    return 'foreign-international';
  }
  if (schoolPropertyLabel.includes('民办双语')) {
    return 'private-bilingual';
  }
  // 荣誉体系（原 tier 的精英层级）
  if (eliteCohort === '四校') return 'si-fen';
  if (eliteCohort === '八大金刚') return 'ba-da';
  if (eliteCohort === '四校分校') return 'si-fen-branch';
  if (eliteCohort === '八大金刚分校') return 'ba-da-branch';
  // 层级
  if (schoolKeyLevel === '市实验性示范性高中') return 'shi-shi-fan';
  if (schoolKeyLevel === '市重点(高中)' || schoolKeyLevel === '市重点') return 'shi-zhongdian';
  if (schoolKeyLevel.includes('特色')) return 'shi-tese';
  if (schoolKeyLevel === '区重点(高中)' || schoolKeyLevel === '区重点') return 'qu-zhongdian';
  if (schoolPropertyLabel.includes('中外合作') || isInternational) return 'guoji-kecheng';
  if (schoolKeyLevel.includes('高中') && schoolPropertyLabel.includes('民办')) return 'minban-gaozhong';
  if (schoolKeyLevel.includes('初中') && schoolPropertyLabel.includes('民办')) return 'minban-chuzhong';
  if (schoolKeyLevel.includes('初中') && schoolPropertyLabel.includes('公办')) return 'gongban-chuzhong';
  if (schoolKeyLevel === '一般高中') return 'yiban-gaozhong';
  if (schoolStage === 'complete') {
    // 完全中学的子分类
    if (schoolPropertyLabel.includes('公办')) return 'gongban-chuzhong';
...
// 从 features 派生「办学星级」（江苏省普通高中星级评估体系：四星/三星/二星/一星）。
// 原始 feature 串常带年份等括号噪声（如「江苏省四星级普通高中（2005年通过验收）」），
// 先剥离括号再匹配，使同一星级归并为单一桶，供苏州学校的「办学星级」筛选维度使用。
const STAR_LEVELS = ['四星级', '三星级', '二星级', '一星级'];
export function deriveSchoolStar(school) {
  const feats = Array.isArray(school?.features) ? school.features : [];
  const norm = feats.map((f) => String(f).replace(/[（(][^）)]*[）)]/g, '').replace(/\s/g, ''));
  for (const level of STAR_LEVELS) {
    if (norm.some((f) => f.includes(level))) return level;
  }
  return '';
}
...
export function inferSpecializations(school) {
  const text = [
    school?.name,
    getSchoolOverview(school),
    school?.admissionInfo?.notes || '',
    getSchoolAchievementsText(school),
