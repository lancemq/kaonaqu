...
// 统计每分类下的学校数
export function countSchoolsByCategory(schools) {
  const counts = new Map();
  for (const s of schools) {
    const cat = s.category || inferSchoolCategory(s);
    counts.set(cat, (counts.get(cat) || 0) + 1);
  }
  return counts;
}

export function countSchoolsBySpecialization(schools) {
  const counts = new Map();
  for (const s of schools) {
    const specs = s.specializations || inferSpecializations(s);
