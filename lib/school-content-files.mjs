...
export function buildSchoolMarkdown(school) {
  ...
  const description = school?.schoolDescription || school?.description || '';
  ...
  blocks.push(
    ...
    `- 办学性质：${ownership}`,
    `- 学校类型：${schoolType}`,
    school?.schoolKeyLevel ? `- 学校属性：${school.schoolKeyLevel}` : null,
...
export async function readSchoolMarkdownFile(itemOrId) {
  const school = typeof itemOrId === 'string' ? null : itemOrId;

  // 优先从 school 对象重建 Markdown（Supabase 的 description 已含 overview）
  if (school) {
    return buildSchoolMarkdown(school);
  }
