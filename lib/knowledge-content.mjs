...
// Lightweight metadata listing for all knowledge pages (used by the channel
// search/filter UI). One read per manifest file is fine at build time.
export async function getAllKnowledgePagesMeta() {
  const manifest = await getPagesManifest();
  const metas = [];
  for (const [slug, filename] of Object.entries(manifest)) {
    try {
      const data = JSON.parse(await fs.readFile(path.join(KNOWLEDGE_DIR, filename), 'utf8'));
      const { subject, grade } = parseSubjectGrade(slug);
      metas.push({
        slug,
        href: data.href || (slug === 'index' ? '/knowledge' : `/knowledge/${slug}`),
        title: data.title || fallbackLabel(slug),
        subject,
        grade,
        subjectLabel: subject ? SUBJECT_LABELS[subject] : null,
        gradeLabel: grade ? GRADE_LABELS[grade] : null,
        category: data.category || '学科知识点',
        tags: data.tags || [],
