...
import { getKnowledgeMeta } from './knowledge-meta.mjs';
import { getQuizzesForSlug } from './knowledge-quizzes.mjs';
...
const SLUG_LABELS = {
  ...
  'chinese-grade8': '八年级语文知识点',
  'math-grade8': '八年级数学知识点',
  'english-grade8': '八年级英语知识点',
  'physics-grade8': '八年级物理知识点',
  'chemistry-grade8': '八年级化学知识点',
  'history-grade8': '八年级历史知识点',
  'politics-grade8': '八年级道德与法治知识点',
  ...
  'geography-grade7': '七年级地理知识点',
  'geography-grade8': '八年级地理知识点',
  'biology-grade7': '七年级生命科学知识点',
  'biology-grade8': '八年级生命科学知识点'
};
...
const GRADE_NAV = {
  grade6: { label: '六年级', desc: '小升初衔接 · 基础巩固' },
...
const SUBJECT_BLURBS = {
  ...
  'history-senior1': '中外历史脉络 · 史料实证 · 核心专题',
  'history-senior2': '选择性必修 · 史料分析 · 专题深化',
  'history-senior3': '高考综合 · 史料论证 · 热点专题',
  'politics-senior1': '政治认同 · 法治意识 · 公共参与',
  'politics-senior2': '哲学与文化 · 法治 · 当代国际',
  'politics-senior3': '高考政治 · 时政论证 · 答题模板',
  'geography-senior1': '自然地理 · 人文地理 · 区域认知',
  'geography-senior2': '区域发展 · 环境保护 · 综合思维',
  'geography-senior3': '高考综合 · 区域地理 · 人地协调',
  'zhongkao-zhenti': '中考真题解析 · 评分标准 · 真题用法',
  'kuaxueke-anli': '跨学科案例分析 · 15分 · 答题结构',
  'tiyu-zhongkao': '体育中考 · 30分 · 项目选择',
  'xuanke-zhidao': '选科指导 6选3 · 合格考 · 专业匹配',
  'geography-grade7': '世界地理 · 地球地图 · 气候',
  'geography-grade8': '中国地理 · 四大区域 · 上海乡土',
  'biology-grade7': '细胞 · 人体系统 · 生理基础',
  'biology-grade8': '健康免疫 · 生态 · 遗传初步'
};
...
export async function buildKnowledgeNav() {
  ...
  const grades = GRADE_ORDER.map((g) => ({
    ...
    href: gradeHref(g),
    isTransition: g === 'grade6',
    disabled: Boolean(GRADE_NAV[g].disabled)
  }));
  const subjectsByGrade = {};
  for (const grade of GRADE_ORDER) {
    if (grade === 'grade6') {
      // 小升初衔接年级无单科页，由结构化年级页承载内容，避免渲染「筹备中」占位
...
export function buildKnowledgeJsonLd(page, region) {
  const BASE = process.env.NEXT_PUBLIC_SITE_URL || 'https://kaonaqu.xyz';
  const url = `${BASE}/${region}${page.href || `/knowledge/${page.slug}`}`;
  const breadcrumb = {
    ...
    itemListElement: (page.breadcrumbItems || []).map((b, i) => ({
      ...
      position: i + 1,
      name: b.label,
      ...(b.href ? { item: `${BASE}/${region}${b.href}` } : {})
    }))
  };
...
export async function getKnowledgePage(slug = []) {
  ...
  const meta = getKnowledgeMeta(normalizedSlug);
  const quizzes = getQuizzesForSlug(normalizedSlug);
    ...
    return {
      ...
      examFrequency: meta?.examFrequency ?? null,
      quizzes: quizzes || []
    };
}
...
export async function listKnowledgeSlugs() {
  const manifest = await getPagesManifest();
  const slugs = new Set(Object.keys(manifest));
  // structured 页（频道/年级总览）即使无对应 JSON 也需纳入预渲染
  Object.keys(knowledgeStructuredPages).forEach((s) => slugs.add(s));
