...
export function getSchoolType(school) {
  return school.schoolPropertyLabel || '未分类';
}
...
export function getSchoolAdmissionInfo(school) {
  // 统一从 admissionInfo 取招生说明文本
  const raw = (typeof school?.admissionInfo?.notes === 'string' && school.admissionInfo.notes)
    || '';
  if (!raw) return '';
...
// 返回结构化的招生信息对象（code/methods/routes/notes），供详情页分块渲染
export function getSchoolAdmissionStructured(school) {
  const info = school?.admissionInfo || {};
  const code = typeof info.code === 'string' ? info.code : (school?.admissionCode || '');
  ...
  const notesRaw = typeof info.notes === 'string' ? info.notes : '';
  const notes = notesRaw
    ? notesRaw.replace(/建议重点关注[^。]*。/g, '').replace(/\s+/g, ' ').trim()
    : '';
  return {
    code: code || '',
