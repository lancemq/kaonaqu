#!/usr/bin/env node
/**
 * 审计 Supabase schools 表的 tier / school_type_label 字段（只读，不写库）
...
function cleanTypeLabel(school) {
  const tl = String(school.school_type_label || '').trim();
  if (tl === '公办') return '公办';
...
function cleanTier(school) {
  const text = `${school.tier || ''} ${school.school_type_label || ''}`;
...
async function fetchAll() {
  ...
  const cols = 'id,slug,name,district_name,school_stage_label,school_type_label,tier,is_international,description';
  const PAGE = 1000;
  let all = [];
  let from = 0;
  while (true) {
...
async function main() {
  ...
  for (const r of rows) {
    ...
    const curType = String(r.school_type_label || '').trim();
    ...
    if (curType && !VALID_TYPES.has(curType)) {
      ...
      reasons.push(`school_type_label 非规范值「${curType}」→「${newType}」`);
    }
    if (newTier !== curTier) reasons.push(`tier 需校正「${curTier || '(空)'}」→「${newTier}」`);
    if (newType !== curType) reasons.push(`school_type_label 需校正「${curType || '(空)'}」→「${newType}」`);
    ...
    if (reasons.length) {
      corrections.push({
        ...
        current_school_type_label: curType,
        proposed_school_type_label: newType,
        reasons
      });
    }
  }
  ...
  const typeChanged = corrections.filter((c) => c.proposed_school_type_label !== c.current_school_type_label).length;
  const nonStdTierRows = corrections.filter((c) => c.current_tier && !CANONICAL_TIERS.has(c.current_tier)).length;
  const nonStdTypeRows = corrections.filter((c) => c.current_school_type_label && !VALID_TYPES.has(c.current_school_type_label)).length;
  console.log('\n━━━ 纠错统计 ━━━');
  console.log(`  需改动行数（tier 或 type 任一变化）: ${corrections.length}`);
  console.log(`    tier 需变更: ${tierChanged}`);
  console.log(`    school_type_label 需变更: ${typeChanged}`);
  ...
  const md = [
    ...
    `- school_type_label 需变更: ${typeChanged}`,
    ...
    ...corrections.slice(0, 200).map((c, i) =>
      `| ${i + 1} | ${c.name} | ${c.district_name || ''} | ${c.school_stage_label || ''} | ${c.current_tier || '(空)'} | ${c.proposed_tier} | ${c.current_school_type_label || '(空)'} | ${c.proposed_school_type_label} | ${c.reasons.join('；')} |`),
