#!/usr/bin/env node
// 读 data/top100-schools.json 和 data/schools.json，
// 按 tier 分档生成 rich profile 模板，输出 lib/school-rich-profiles.generated.js。
//
// 设计原则（已与 user 确认）：
...
//
// 重跑场景：top100-schools.json 变化或模板规则调整。
...
const TOP100_PATH = path.join(ROOT, 'data', 'top100-schools.json');
...
function serialize(profiles) {
  ...
  lines.push('// Do not edit by hand. Re-run the script when top100 list or template rules change.');
...
function main() {
  ...
  const top100 = JSON.parse(fs.readFileSync(TOP100_PATH, 'utf8'));
  const byId = new Map(schools.map((s) => [s.id, s]));

  const out = {};
  let missing = 0;
  for (const t of top100) {
    const school = byId.get(t.id);
    if (!school) {
      missing += 1;
      console.warn(`  ! school not found in schools.json: ${t.id}`);
      continue;
    }
    out[t.id] = buildProfile(school);
  }

  fs.writeFileSync(OUT_PATH, serialize(out));
  console.log(`Generated ${Object.keys(out).length} profiles → ${path.relative(ROOT, OUT_PATH)}`);
  if (missing) console.log(`  ! ${missing} ids missing in schools.json`);
}
